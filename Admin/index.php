...
$stats = [
    ...
    'pending_exams' => $con->query("SELECT COUNT(*) as count FROM exams WHERE approval_status='pending'")->fetch_assoc()['count'] ?? 0,
...
$recent_approvals = $con->query("
    SELECT es.exam_name, c.course_code, es.approval_status, 
           COALESCE(es.approved_at, es.updated_at) as updated_at
    FROM exams es 
    JOIN courses c ON es.course_id = c.course_id 
    WHERE es.approval_status IN ('approved', 'rejected')
    ORDER BY COALESCE(es.approved_at, es.updated_at) DESC 
    LIMIT 5
");
?>
<!DOCTYPE html>
