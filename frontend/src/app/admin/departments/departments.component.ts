import { Component, OnInit } from '@angular/core';
import { CommonModule } from '@angular/common';
import { FormsModule } from '@angular/forms';
import { ApiService } from '../../core/services/api.service';

@Component({
    selector: 'app-departments',
    standalone: true,
    imports: [CommonModule, FormsModule],
    templateUrl: './departments.component.html'
})
export class DepartmentsComponent implements OnInit {
    departments: any[] = [];
    faculties: any[] = [];
    loading = true;
    showForm = false;
    editing: any = null;
    form: any = {};
    error = '';

    constructor(private api: ApiService) { }

    ngOnInit() {
        this.api.getFaculties().subscribe(f => this.faculties = f);
        this.load();
    }

    load() {
        this.loading = true;
        this.api.getDepartments().subscribe({
            next: d => { this.departments = d; this.loading = false; },
            error: () => { this.loading = false; }
        });
    }

    openCreate() { this.editing = null; this.form = {}; this.showForm = true; this.error = ''; }
    openEdit(d: any) { this.editing = d; this.form = { ...d }; this.showForm = true; this.error = ''; }

    save() {
        const obs = this.editing
            ? this.api.updateDepartment(this.editing.id, this.form)
            : this.api.createDepartment(this.form);
        obs.subscribe({
            next: () => { this.showForm = false; this.load(); },
            error: err => { this.error = JSON.stringify(err.error); }
        });
    }

    delete(id: number) {
        if (!confirm('Delete this department?')) return;
        this.api.deleteDepartment(id).subscribe(() => this.load());
    }
}
